#####3. plotting study area for predators 
library(cowplot)
library(ggspatial)
library(tidyverse)
...
ARS <- read.csv(file = "Raw Data/CoordsCovsARS.csv", header = TRUE) %>%  
  st_as_sf(coords = c("x", "y"), crs = "epsg:7842")%>%
  filter(Habitat == "Urban")

trapsuniqueplot <- traps #from script for secr objects.
trapsuniqueplot <- st_as_sf(trapsuniqueplot, coords =c("x", "y"), crs = st_crs(ARS))



plotfixed <- st_read("Raw Data/studyareaclipped.shp")
